...
[] execVM "INS_revive\gear\KRON_Strings.sqf";
...
fnc_get_loadout = compile preprocessFileLineNumbers 'INS_revive\loadout\fnc_get_loadout.sqf';
fnc_set_loadout = compile preprocessFileLineNumbers 'INS_revive\loadout\fnc_set_loadout.sqf';  

fnc_save_gear = 
{
	//private["_dialog","_list","_slist","_edit","_sel","_primary,_launcher","_handgun","_magazines","_uniform","_vest","_backpack","_items","_primitems","_secitems","_handgunitems"];
	private["_dialog","_list","_slist","_edit","_sel","_loadout"];
	...
	_loadout=[player] call fnc_get_loadout;
	profileNameSpace setVariable[format["vas_gear_naong_%1",_sel],[_edit, _loadout]];
	
	//[0] execVM "INS_revive\gear\refresh.sqf";
};

fnc_load_gear =
{
	//private["_dialog","_list","_edit","_sel","_primary,_launcher","_handgun","_magazines","_uniform","_vest","_backpack","_items","_primitems","_secitems","_handgunitems","_item_type"];
	private["_dialog","_list","_edit","_sel","_loadout"];
	disableSerialization;
	...
	if(!isnil {profileNameSpace getVariable format["vas_gear_naong_%1",_sel]}) then
	{
		_loadout = (profileNameSpace getVariable format["vas_gear_naong_%1",_sel]) select 1;
		[player, _loadout] call fnc_set_loadout;
	};
};
...
fnc_save_gear = 
{
	...
	player selectWeapon (primaryWeapon player);
	profileNameSpace setVariable[format["vas_gear_naong_%1",_sel],[_edit,_primary,_launcher,_handgun,_magazines,_uniform,_vest,_backpack,_items,_primitems,_secitems,_handgunitems,_uitems,_vitems,_bitems]];
...
fnc_delete_gear =
{
	...
	_dialog = findDisplay 2520;
	_sel = lbCurSel 2521;
	profileNameSpace setVariable[format["vas_gear_naong_%1",_sel],nil];
	[1] execVM "INS_revive\gear\refresh.sqf";
};
/*
fnc_load_gear =
{
	...
	if(!isnil {profileNameSpace getVariable format["vas_gear_naong_%1",_sel]}) then
	{
		_loadout = profileNameSpace getVariable format["vas_gear_naong_%1",_sel];
...
vas_loadonrespawn =
{
	...
	{
		if(!isnil {profileNameSpace getVariable format["vas_gear_naong_%1",vas_loadout]}) then
		{
			_loadout = profileNameSpace getVariable format["vas_gear_naong_%1",vas_loadout];
			_primary = _loadout select 1;
			_launcher = _loadout select 2;
			_handgun = _loadout select 3;
			_magazines = _loadout select 4;
	...
	{
		{
			...
			{
				if(!(_classname in vas_r_backpack)) then
				{
...
// Function - add items to listbox
FNC_Add_Loadout_to_Listbox = {
	private ["_dialogID","_dialog","_profile","_listboxID","_listbox","_loadout","_loadout_array","_name","_pic","_assignedItems","_primaryWeapon","_primaryWeaponItems","_handgunWeapon","_handgunItems","_secondaryWeapon","_secondaryWeaponItems","_uniform","_uniformItems","_vest","_vestItems","_backpack","_backpackItems","_headgear","_goggles","_className","_listItems"];
	
	_dialogID 	= _this select 0;	// Dialogbox ID
	_listboxID 	= _this select 1;	// Listbox ID
	_profile 	= _this select 2;	// Loadout Data
	
	_dialog = findDisplay _dialogID;
	_listbox = _dialog displayCtrl _listboxID;
	
	if (!isnil "_profile") then {
		CtrlShow[_listboxID, true];
		_loadout = (_profile) select 1;
		_loadout_array = [];
		
		_assignedItems 		= _loadout select 0;

		_primaryWeapon 		= _loadout select 1;
		_primaryWeaponItems = [];
		_primaryWeaponItems = _primaryWeaponItems + _loadout select 2;

		_handgunWeapon 		= _loadout select 3;
		_handgunItems 		= [];
		_handgunItems 		= _handgunItems + _loadout select 4;

		_secondaryWeapon 		= _loadout select 5;
		_secondaryWeaponItems 	= [];
		_secondaryWeaponItems 	= _secondaryWeaponItems + _loadout select 6;

		_uniform 			= _loadout select 7;
		_uniformItems 		= [];
		_uniformItems		= _uniformItems + _loadout select 8;

		_vest 				= _loadout select 9;
		_vestItems 			= [];
		_vestItems 			= _vestItems + _loadout select 10;

		_backpack 			= _loadout select 11;
		_backpackItems 		= [];
		_backpackItems 		= _backpackItems + _loadout select 12;

		_headgear 			= _loadout select 13;
		_goggles 			= _loadout select 14;
		
		_loadout_array = _loadout_array + [_primaryWeapon] + _primaryWeaponItems + [_secondaryWeapon] + _secondaryWeaponItems + [_handgunWeapon] + _handgunItems + _uniformItems + _vestItems + _backpackItems + 
						 [_uniform] + [_vest] + [_backpack] + _assignedItems;
		...
		_listItems = [];
		{
			...
			if (isClass (configFile >> "CfgWeapons" >> _x)) then {
				...
				_name = format["x%1 %2",({_x == _className} count _loadout_array),_name];
			...
			// Do not add dublication.
			if (_listItems find _name > -1) then {_name = ""};
	
			// Add to listbox
			if (_name != "") then {
				//player sidechat format["Debug : Test %1",isNil "_listbox"];
